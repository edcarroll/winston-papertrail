...
Papertrail.prototype.log = function (level, msg, meta, callback) {
    ...
    if  (meta && typeof meta === 'object' && Object.getOwnPropertyNames(meta).length == 0) {
        meta = false;
    }
    ...
    if (this.inlineJson) {
        output = { message: msg, level: level ? level : 'error', timestamp: (new Date()).getTime() };
    }
    ...
    if (meta) {
        if (typeof meta !== 'object') {
            ...
            if (this.inlineMeta) {
                ...
                output.meta = winston.clone(meta) || {};
            }
            else {
    ...
    if (this.inlineJson) {
        try {
            ...
            output.meta = { error: 'Logger cannot convert object structure to JSON.' };
            output = JSON.stringify(output);
        }
    }

    if (typeof(output) !== 'string') {
        output = util.inspect(output, false, null, self.colorize).replace(/[\n\t]\s*/gm, " ");
    }

    this.sendMessage(this.hostname, this.program, this.inlineJson ? 'info' : level, output);
 ...
 *
 * @param {String}    program    Hostname of the source application.
 * @param {String}    hostname     Name of the source application
 * @param {String}    level        Log level of the message
 * @param {String}    message        The message to deliver
 */
Papertrail.prototype.sendMessage = function (program, hostname, level, message) {
